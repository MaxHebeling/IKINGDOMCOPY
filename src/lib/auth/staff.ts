// Canonical staff auth guard — mirrored from hebeling-imperium-hub/lib/auth/staff.ts
 ...
 * Throws "UNAUTHORIZED" if no session, "FORBIDDEN" if role is not staff.
 */
export async function requireStaff(): Promise<StaffSession> {
  ...
  if (!STAFF_ROLES.includes(profile.role as StaffRole)) throw new Error("FORBIDDEN");
