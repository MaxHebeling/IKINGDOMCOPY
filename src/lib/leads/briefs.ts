// Canonical briefs — mirrored from hebeling-imperium-hub/lib/leads/briefs.ts
// storeLeadBrief() is the canonical iKingdom brief intake function.
...
async function updateLeadFromBrief(lead: Lead, payload: LeadBriefPayload) {
  ...
  if (error) throw new Error(`Failed to update lead from brief: ${error.message}`);
}
...
export async function storeLeadBrief(
  ...
  const submittedAt = payload.fecha
  ...
  if (error) throw new Error(`Failed to store lead brief: ${error.message}`);
